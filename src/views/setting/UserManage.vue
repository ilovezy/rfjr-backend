    ...
    <div class='main-table-wrap'>
      <el-table
        ...
        <el-table-column
          ...
          label="操作">
          <template slot-scope="scope">
            <router-link v-permission='["platform:user:view"]'
                         :to="{path: '/setting/UserManageEdit/'+scope.row.id, query: {isPreview: true}}">
              <el-button size="mini">
                查看
              </el-button>
            </router-link>
            <router-link v-permission='["platform:user:edit"]'
            ...
            <el-button size='mini'
                       type='primary'
                       v-permission='["platform:user:freeze"]'
            ...
            <el-button size='mini'
                       type='warning'
                       v-permission='["platform:user:unfreeze"]'
                       @click="handleToggleFreeze(scope.$index, scope.row, scope.row.status == 'freeze')"
                       v-if='scope.row.status == "freeze" && scope.row.status != "cancel"'>解冻
            </el-button>

            <el-button
              v-permission='["platform:user:cancel"]'
              v-if='scope.row.status != "cancel"'
              size="mini"
              type="danger"
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        AXIOS.post('/a/platform/user/index', self.listQuery).then(res => {
      ...
      handleToggleFreeze(index, row, freeze) {
        ...
        const url = freeze ? '/a/platform/user/unfreeze' : '/a/platform/user/freeze'
      ...
      handleCancel(index, row) {
        this.$confirm('确定注销？', {
          ...
          AXIOS.post('/a/platform/user/cancel', {
            id: row.id
          }).then(res => {
            this.$message({
              type: 'success',
